...
// ── Topbar ─────────────────────────────────────────────────────
export const Topbar: React.FC<TopbarProps> = ({ toggleSidebar, isDesktop, isMobile }) => {
    ...
    return (
        <header style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                {/* Busca apenas desktop/tablet */}
                {isDesktop && <GlobalSearch />}
                {/* No mobile: mostrar logo no centro */}
                {isMobile && (
            ...
            {/* Direita: notificações + usuário + sair */}
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '14px' }}>
                <button style={{
                    color: 'rgba(255,255,255,0.6)', cursor: 'pointer', position: 'relative',
                    background: 'none', border: 'none', padding: '8px', minHeight: '44px', minWidth: '44px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                    <Bell size={20} />
                    <span style={{ position: 'absolute', top: '6px', right: '6px', width: '7px', height: '7px', background: 'var(--accent-primary)', borderRadius: '50%' }} />
                </button>

                {/* Nome do usuário — oculto no iPhone */}
                {!isMobile && (
                    <>
                        <div style={{ height: '24px', width: '1px', background: 'var(--border-color)' }} />
                        <div style={{ textAlign: 'right' }}>
                            <span style={{ fontSize: '14px', fontWeight: 600, color: '#fff', lineHeight: 1.2, display: 'block' }}>
                                {user?.name || 'User'}
                            </span>
                            <span style={{ fontSize: '11px', color: isSuperAdmin ? 'var(--accent-primary)' : 'rgba(255,255,255,0.45)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                ...
                <button
                    onClick={signOut}
                    style={{
                        padding: '8px', color: 'rgba(255,255,255,0.6)', borderRadius: '10px',
                        background: 'var(--glass)', display: 'flex', alignItems: 'center',
